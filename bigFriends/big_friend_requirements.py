...
def big_friend_juftligi(requirement):
    """Ikkinchi sonni katta do'st konsepsiyasiga muvofiq tanlaydi."""
    operation, number = requirement[0], int(requirement[1])
    if operation == '+':
        if number == 9:
            return random.choice([1, 2, 3, 4, 6, 7, 8, 9])
        elif number == 8:
            return random.choice([2, 3, 4, 7, 8, 9])
        elif number == 7:
        ...
        if number == 9:
            return random.choice([10, 15, 16, 17, 18])
        elif number == 8:
            return random.choice([10, 15, 16, 17])
        elif number == 7:
            return random.choice([10, 15, 16])
        elif number == 6:
...
def generate_example(column, requirement, digits=1):
    """katta do'st konsepsiyasi bilan numbers yaratadi va oraliq natijalarni tekshiradi."""
    ...
    while True:  # Ustunlar soni to'g'ri bo'lmaguncha takrorlaymiz
        ...
        if requirement and digits == 1:
            ...
            first_number = big_friend_juftligi(requirement)
        else:
            operation = random.choice(['-', '+'])
            second_number = random.choice([1, 2, 3, 4])
            first_number = big_friend_juftligi(operation + str(second_number))
...
def big_friend(column=5, requirement='+1', count=1):
...
if __name__ == '__main__':
    start = datetime.now()
    for _ in range(1):
        pprint.pprint(big_friend(column=5, requirement='-5', count=10))
    end = datetime.now()
